...
#Сохранение отправленного и полученного сигнала
...
sdr.rx_buffer_size =2*n_frame*4# размер буфера увеличен в 8 раз по сравнению с отправленным сигналом
xrec1=sdr.rx()
sdr.tx_destroy_buffer()
...
#Быстрое преоббразование Фурье (2048 - для большей точности)
...
index_max_elem = np.argmax(sig_fft)#индекс элемента с наибольшей частотой
print("index_max_elem =", index_max_elem)
